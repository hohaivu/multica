...
| `MULTICA_TASK_QUEUED_TTL` | `2h` | 태스크가 대기열에 점유되지 않고 머물 수 있는 최대 시간. 초과 시 `queued_expired`로 실패. 저 동시성 런타임이 긴 작업 뒤에 합리적으로 대기할 수 있으므로 이 경우 값을 키워야 함 |
...
| GitLab OAuth | `MULTICA_GITLAB_INSTANCE_URL` | GitLab 인스턴스 URL |
| GitLab OAuth | `MULTICA_GITLAB_OAUTH_CLIENT_ID` | OAuth 클라이언트 ID |
| GitLab OAuth | `MULTICA_GITLAB_OAUTH_CLIENT_SECRET` | OAuth 클라이언트 시크릿 |
...
| `MULTICA_AGENT_IDLE_WATCHDOG` | `3m` | 출력과 도구 실행이 모두 없을 때의 무응답 상한 |
| `MULTICA_AGENT_TOOL_WATCHDOG` | `2h` | 단일 도구 호출이 계속 무응답인 시간 상한 |
| `MULTICA_OPENCODE_IDLE_WATCHDOG` | `25m` | OpenCode 전용 무응답 기준값 |
