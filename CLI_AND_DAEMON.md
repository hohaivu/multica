...
| Agent idle watchdog | — | `MULTICA_AGENT_IDLE_WATCHDOG` | `3m` (`0` disables the whole watchdog suite) |
| Agent tool watchdog | — | `MULTICA_AGENT_TOOL_WATCHDOG` | `2h` (`0` = never force-stop during a tool call) |
| Codex semantic inactivity timeout | `--codex-semantic-inactivity-timeout` | `MULTICA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | same as the tool watchdog (Codex's timer is not tool-aware, so it tracks the larger of the idle / tool budgets) |
...
| OpenCode idle watchdog | — | `MULTICA_OPENCODE_IDLE_WATCHDOG` | `25m` (opencode's run-long silence budget, in place of the 2h tool watchdog; `0` falls back to it) |
