...
// A .git entry that IS present but that git itself cannot read (corrupt,
// unsupported format, dubious ownership, ...) must fail closed like any other
// unprotectable repository — not be treated as a plain folder just because
// `git rev-parse` returned an error. See #7114.
func TestGitExcludesFailClosedWhenGitDirIsUnreadable(t *testing.T) {
	dir := t.TempDir()
	envRoot := t.TempDir()
	sidecar := mkSidecarDir(t, dir, ".multica")
	// A `.git` file (not a directory) with content git does not understand as
	// a gitdir pointer makes `git rev-parse --show-toplevel` fail while a
	// `.git` entry genuinely exists.
	writeFile(t, filepath.Join(dir, ".git"), "not a real gitdir pointer\n")

	_, err := PrepareGitExcludes(envRoot, dir, []string{sidecar})
	if !errors.Is(err, ErrGitExcludesUnprotected) {
		t.Fatalf("expected ErrGitExcludesUnprotected, got %v", err)
	}
}
...
func TestGitExcludesPreserveTheUsersGlobalIgnores(t *testing.T) {
	repo := newTestRepo(t)
	envRoot := t.TempDir()
	// Isolate from the machine's real ~/.gitconfig: it commonly sets
	// core.excludesFile itself (e.g. via Sourcetree/GitHub Desktop), and
	// that setting is read AFTER XDG's in git's own precedence order —
	// masking the fake one this test sets up below and making the test
	// pass or fail depending on the developer's own dotfiles rather than
