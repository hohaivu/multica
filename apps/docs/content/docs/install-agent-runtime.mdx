...
Antigravity ACP is different: it isn't a CLI Multica detects on `PATH`. It's Google's ACP server for Antigravity (`agy_acp_server.par`), a separate ~300MB download from the `agy` CLI above, and the daemon installs it for you:

```bash
multica runtime install antigravity-acp   # one-time ~300MB download; Apple Silicon, Linux, or Windows only — no darwin-x86_64 build
multica runtime auth antigravity-acp      # one-time interactive Google login (opens a browser)
```

`install` never authenticates by itself, and `auth` never runs automatically — run it yourself at a terminal that can reach a browser. The daemon registers the runtime only once both the binary and credentials (from `runtime auth`, or `GEMINI_API_KEY`/`GOOGLE_API_KEY` in the environment) are present.
