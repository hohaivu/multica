...
| `MULTICA_AGENT_IDLE_WATCHDOG` | `3m` | Silence ceiling with no output and no tool execution; `0` disables the watchdog entirely |
| `MULTICA_AGENT_TOOL_WATCHDOG` | `2h` | Silence ceiling for a single tool call; `0` never force-stops a tool call |
| `MULTICA_OPENCODE_IDLE_WATCHDOG` | `25m` | OpenCode's run-long silence budget (replaces the tool watchdog, since it cannot report in-flight tools) |
| `MULTICA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | same as `MULTICA_AGENT_TOOL_WATCHDOG` | Codex semantic-silence threshold. Codex's own timer cannot see that a tool is running, so it tracks the larger of the idle / tool budgets instead of holding a separate, shorter ceiling |
