...
func TestGCWorkspace_BatchMissingResultDoesNotClean(t *testing.T) {
	issueID := "77777777-7777-7777-7777-777777777778"

	mux := http.NewServeMux()
	mux.HandleFunc("/api/daemon/workspaces/ws-missing/issues/gc-check", func(w http.ResponseWriter, r *http.Request) {
	...
	d.cfg.GCCompletedTaskTTL = time.Hour
	d.cfg.GCArtifactTTL = 0
	wsDir := filepath.Join(d.cfg.WorkspacesRoot, "ws-missing")
	taskDir := createTaskDir(t, d.cfg.WorkspacesRoot, "ws-missing", "task", &execenv.GCMeta{
		Kind: execenv.GCKindIssue, IssueID: issueID, WorkspaceID: "ws-missing",
		CompletedAt: time.Now().Add(-2 * time.Hour),
	})
	stats := &gcStats{byPattern: map[string]int{}}
	d.gcWorkspace(context.Background(), wsDir, stats)
	...
	if _, err := os.Stat(taskDir); err != nil {
		t.Fatalf("task dir must remain when batch result is missing: %v", err)
	}
	if stats.cleaned != 0 || stats.orphaned != 0 || stats.skipped != 1 {
		t.Fatalf("unexpected stats after missing result: %+v", stats)
	}
}
...
func TestShouldCleanTaskDir_CompletedTaskTTLRemovesOpenIssue(t *testing.T) {
	...
	mux.HandleFunc(fmt.Sprintf("/api/daemon/issues/%s/gc-check", issueID), func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]any{
			"status": "in_review", "updated_at": time.Now(),
		})
	})
...
func TestShouldCleanTaskDir_CompletedTaskTTLRequiresCompletionTime(t *testing.T) {
	...
	mux := http.NewServeMux()
	mux.HandleFunc(fmt.Sprintf("/api/daemon/issues/%s/gc-check", issueID), func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(map[string]any{"status": "blocked", "updated_at": time.Now()})
	})
...
func TestShouldCleanTaskDir_CompletedTaskTTLRejectsEmptyStatus(t *testing.T) {
	t.Parallel()
	issueID := "88888888-8888-8888-8888-888888888884"

	mux := http.NewServeMux()
	mux.HandleFunc(fmt.Sprintf("/api/daemon/issues/%s/gc-check", issueID), func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(map[string]any{"status": "  ", "updated_at": time.Now()})
	})

	d := newGCTestDaemon(t, mux)
	d.cfg.GCCompletedTaskTTL = time.Hour
	d.cfg.GCArtifactTTL = 0
	taskDir := createTaskDir(t, d.cfg.WorkspacesRoot, "ws1", "empty-status", &execenv.GCMeta{
		Kind: execenv.GCKindIssue, IssueID: issueID, WorkspaceID: "ws1",
		CompletedAt: time.Now().Add(-2 * time.Hour),
	})

	if action := d.shouldCleanTaskDir(context.Background(), taskDir); action != gcActionSkip {
		t.Fatalf("expected empty status to fail closed, got %d", action)
	}
}
...
func TestShouldCleanTaskDir_CompletedTaskTTLKeepsLocalDirectoryArtifactTTLSeparate(t *testing.T) {
	...
	taskDir := createTaskDir(t, d.cfg.WorkspacesRoot, "ws1", "local-directory-recent", &execenv.GCMeta{
		Kind: execenv.GCKindIssue, IssueID: issueID, WorkspaceID: "ws1",
		CompletedAt: time.Now().Add(-2 * time.Hour), LocalDirectory: true,
	})
	...
	artifactEligibleDir := createTaskDir(t, d.cfg.WorkspacesRoot, "ws1", "local-directory-artifact-eligible", &execenv.GCMeta{
		Kind: execenv.GCKindIssue, IssueID: issueID, WorkspaceID: "ws1",
		CompletedAt: time.Now().Add(-13 * time.Hour), LocalDirectory: true,
	})
...
func TestShouldCleanTaskDir_CompletedTaskTTLPreservesActiveEnv(t *testing.T) {
	...
	mux := http.NewServeMux()
	mux.HandleFunc(fmt.Sprintf("/api/daemon/issues/%s/gc-check", issueID), func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(map[string]any{"status": "todo", "updated_at": time.Now()})
	})
