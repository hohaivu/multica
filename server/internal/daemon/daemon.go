...
const (
	...
	// slot for 3h. The cap makes worst-case detection window + this tick no
	// matter how large an operator sets the budget.
	//
	// Upstream sets this to 5m, sized as the largest overshoot worth tolerating
	// on top of a budget measured in hours. This fork's default idle budget is
	// 3m (DefaultAgentIdleWatchdog, VUH-132), against which a 5m ceiling never
	// engages and window/2 alone would poll every 90s — a 50% overshoot, which
	// is exactly the idle_for=10m58s-against-a-10m-window symptom VUH-132 was
	// filed for. 30s keeps the overshoot bounded for short budgets and still
	// costs nothing on long ones: a tick is an atomic load and a channel length
	// check.
	idleWatchdogMaxTick = 30 * time.Second
)
...
// qwenpaw.go, traecli.go, openclaw.go, codebuddy.go and dim.go. Everything
// else is absent deliberately: claude, pi, opencode, deveco and hermes each
// document at their own call site why they drop the field, and a backend that
// ignored it would leave the agent with no brief at all — which is why the
// default is false and a new runtime has to opt in consciously.
func providerSupportsInlineSystemPrompt(provider string) bool {
	...
	switch provider {
	case "codebuddy", "dim", "grok", "kimi", "kiro", "openclaw", "qoder", "qoderclicn", "qwenpaw", "traecli":
		return true
	default:
		return false
	}
