...
| `MULTICA_TASK_QUEUED_TTL` | `2h` | タスクがキュー内でクレームされずに置かれることのできる最長時間。超過すると `queued_expired` で失敗します。低並行度のランタイムが長時間実行中の作業の後ろに正当に待機しうる場合は大きくしてください |
...
| GitLab OAuth | `MULTICA_GITLAB_INSTANCE_URL` | GitLab インスタンス URL |
| GitLab OAuth | `MULTICA_GITLAB_OAUTH_CLIENT_ID` | OAuth クライアント ID |
| GitLab OAuth | `MULTICA_GITLAB_OAUTH_CLIENT_SECRET` | OAuth クライアントシークレット |
...
| `MULTICA_AGENT_IDLE_WATCHDOG` | `3m` | 出力もツール実行もない状態の静穏上限 |
| `MULTICA_AGENT_TOOL_WATCHDOG` | `2h` | 単一のツール呼び出しが静穏なままでいられる上限 |
| `MULTICA_OPENCODE_IDLE_WATCHDOG` | `25m` | OpenCode 専用の静穏しきい値 |
...
各 AI コーディングツールは `MULTICA_<PROVIDER>_PATH` でコマンドパスを上書きでき、モデル上書き対応ツールでは `MULTICA_<PROVIDER>_MODEL` も利用できます。QwenPaw と MiniMax Code にはモデル変数がなく、MiniMax Code のパス変数は `MULTICA_MCODE_PATH` です。詳しくは [AI コーディングツール比較](/providers#モデルの取得元) を参照してください。DeepSeek Harness は `MULTICA_DSH_PATH` と `MULTICA_DSH_MODEL` に対応しています（値は dsh のモデルカタログにあるモデル ID、例: `deepseek-official/deepseek-chat`）。ZeroClaw は `MULTICA_ZEROCLAW_PATH` に対応しますが、モデル変数はありません。モデルは ZeroClaw のエージェント設定で管理します。Antigravity ACP は、デーモン管理のインストールを迂回して手動インストールした `agy_acp_server.par` を使うための `MULTICA_ANTIGRAVITY_ACP_PATH` に対応しますが、モデル選択に対応していないためモデル変数はありません。詳しくは [AI コーディングツールをインストールする](/install-agent-runtime) を参照してください。マシン全体のデフォルト引数 `MULTICA_<PROVIDER>_ARGS` は、現在 Claude Code、Codex、CodeBuddy、Qwen Code、QwenPaw の 5 ツールに対応しています。対応する変数は `MULTICA_CLAUDE_ARGS`、`MULTICA_CODEX_ARGS`、`MULTICA_CODEBUDDY_ARGS`、`MULTICA_QWEN_ARGS`、`MULTICA_QWENPAW_ARGS` です。例:
