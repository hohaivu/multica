...
| `MULTICA_TASK_QUEUED_TTL` | `2h` | task 在队列中无人认领的最长时长，超过后以 `queued_expired` 失败；低并发运行时可能合理地排在长任务之后，此时应调大 |
...
| GitLab OAuth | `MULTICA_GITLAB_INSTANCE_URL` | GitLab 实例 URL |
| GitLab OAuth | `MULTICA_GITLAB_OAUTH_CLIENT_ID` | OAuth 客户端 ID |
| GitLab OAuth | `MULTICA_GITLAB_OAUTH_CLIENT_SECRET` | OAuth 客户端密钥 |
...
| `MULTICA_AGENT_IDLE_WATCHDOG` | `3m` | 没有输出且没有工具执行时的静默上限 |
| `MULTICA_AGENT_TOOL_WATCHDOG` | `2h` | 单个工具调用持续静默的上限 |
| `MULTICA_OPENCODE_IDLE_WATCHDOG` | `25m` | OpenCode 专用静默阈值 |
