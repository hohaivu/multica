...
Antigravity ACP is a separate ~300 MB Google download (`agy_acp_server.par`), not the `agy` CLI, and there is no build for Intel Macs. Install it with `multica runtime install antigravity-acp`, then complete one interactive Google login before the daemon registers it as a healthy runtime; see [Install AI coding tools](/install-agent-runtime).
