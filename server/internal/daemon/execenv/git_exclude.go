...
func PrepareGitExcludes(envRoot, workDir string, paths []string) (*GitProtection, error) {
	...
	if !ok {
		// gitRepoRoot's !ok also covers "git failed for a repository that IS
		// there" (missing/broken git, dubious ownership, a corrupt repo) —
		// exactly the condition this function exists to fail closed on. Only
		// the absence of any .git above workDir is a genuine plain folder.
		if hasGitDirAbove(workDir) {
			return nil, fmt.Errorf("%w: could not determine the repository root", ErrGitExcludesUnprotected)
		}
		return nil, nil
	}
	if envRoot == "" {
...
// hasGitDirAbove reports whether a .git entry exists at or above workDir,
// walking up without shelling out to git. Used only to tell "no repository
// here" apart from "there is one but git could not read it" when
// gitRepoRoot fails, so the latter can fail closed instead of silently
// no-opping.
func hasGitDirAbove(workDir string) bool {
	dir := workDir
	for {
