...
Register a confidential GitLab OAuth application with scope `api` and redirect URI `{MULTICA_PUBLIC_URL}/api/gitlab/oauth/callback`, then set the GitLab OAuth environment variables and click **Connect GitLab**. Project webhooks work on Free/Community Edition; group webhooks require GitLab Premium. The token acts as the authorizing user, and an OAuth connection upgrades an existing PAT connection for the same workspace and instance in place.
