...
| `MULTICA_AGENT_IDLE_WATCHDOG` | `3m` | 没有输出且没有工具执行时的静默上限；`0` 表示整套 watchdog 关闭 |
| `MULTICA_AGENT_TOOL_WATCHDOG` | `2h` | 单个工具调用持续静默的上限；`0` 表示工具执行期间永不强制停止 |
| `MULTICA_OPENCODE_IDLE_WATCHDOG` | `25m` | OpenCode 贯穿整次运行的静默预算（它无法报告执行中的工具，因此用它替代工具 watchdog） |
| `MULTICA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | 同 `MULTICA_AGENT_TOOL_WATCHDOG` | Codex 语义静默阈值。Codex 自己的计时器看不到工具是否在执行，因此跟随 idle / tool 预算中较大的那个，而不再单独保留一个更短的上限 |
