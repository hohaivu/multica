...
| `MULTICA_TASK_QUEUED_TTL` | `2h` | How long a task may sit in the queue unclaimed before failing with `queued_expired`; raise it when low-concurrency runtimes can legitimately wait behind long-running work |
...
| GitLab OAuth | `MULTICA_GITLAB_INSTANCE_URL` | GitLab instance URL |
| GitLab OAuth | `MULTICA_GITLAB_OAUTH_CLIENT_ID` | OAuth client ID |
| GitLab OAuth | `MULTICA_GITLAB_OAUTH_CLIENT_SECRET` | OAuth client secret |
...
| `MULTICA_AGENT_IDLE_WATCHDOG` | `3m` | Silence ceiling with no output and no tool execution |
