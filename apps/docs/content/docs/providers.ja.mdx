...
Antigravity ACP は `agy` CLI とは別の、Google が提供する約 300 MB のダウンロード（`agy_acp_server.par`）で、Intel Mac 向けのビルドはありません。`multica runtime install antigravity-acp` でインストールしたあと、1 回の対話的な Google ログインを完了させるまで、daemon はこれを正常なランタイムとして登録しません。詳しくは [AI コーディングツールのインストール](/install-agent-runtime) を参照してください。
