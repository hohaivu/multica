...
スコープ `api`、リダイレクト URI `{MULTICA_PUBLIC_URL}/api/gitlab/oauth/callback` の Confidential GitLab OAuth アプリを登録し、GitLab OAuth 環境変数を設定して **GitLab を接続**をクリックします。プロジェクト Webhook は Free/Community Edition で利用でき、グループ Webhook には GitLab Premium が必要です。トークンは認可ユーザーとして動作し、同じワークスペースとインスタンスの OAuth 接続は既存の PAT 接続をその場でアップグレードします。
