...
type Task struct {
	...
	WorkDirReused                 bool                   `json:"-"`                                          // daemon-local: PriorWorkDir was actually reused for this run (execenv.Reuse can decline and fall back to a fresh Prepare). Never sent by the server; set by the daemon before BuildPrompt.
