...
| `MULTICA_AGENT_IDLE_WATCHDOG` | `3m` | 出力もツール実行もない状態の静穏上限。`0` で watchdog 全体を無効化 |
| `MULTICA_AGENT_TOOL_WATCHDOG` | `2h` | 単一のツール呼び出しが静穏なままでいられる上限。`0` はツール実行中に強制停止しません |
| `MULTICA_OPENCODE_IDLE_WATCHDOG` | `25m` | OpenCode の実行全体にわたる静穏予算（in-flight のツールを報告できないため、ツール watchdog の代わりに使われます） |
| `MULTICA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `MULTICA_AGENT_TOOL_WATCHDOG` と同じ | Codex のセマンティック静穏しきい値。Codex 自身のタイマーはツール実行中かどうかを判別できないため、独自の短い上限を持たず idle / tool 予算の大きい方に追従します |
