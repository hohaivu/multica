...
<Callout type="info">
Forgejo/Gitea and GitLab's PAT fallback store an instance URL plus token and require a manually registered webhook. GitLab OAuth authorizes once and lets Multica register project or group hooks for you. Credentials and webhook secrets are encrypted at rest.
</Callout>

### Connect GitLab in one click

Register a confidential GitLab OAuth application with scope `api` and redirect URI `{MULTICA_PUBLIC_URL}/api/gitlab/oauth/callback`, then set `MULTICA_GITLAB_INSTANCE_URL`, `MULTICA_GITLAB_OAUTH_CLIENT_ID`, `MULTICA_GITLAB_OAUTH_CLIENT_SECRET`, and `MULTICA_PUBLIC_URL` before clicking **Connect GitLab**. Register it instance-wide under **Admin → Applications**, or as a user-owned app under **User → Settings → Applications**. Project webhooks work on Free/Community Edition; group webhooks require GitLab Premium.

The `api` scope grants broad full read/write access as the authorizing user, strictly more than today's `read_api` PAT. GitLab has no narrower scope that can create webhooks. Use one GitLab instance per deployment; connecting OAuth upgrades an existing PAT connection for the same workspace and instance in place.
